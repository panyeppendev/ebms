<script>
	...
	const doEmergency = el => {
		Swal.fire({
			title: 'Yakin, nih?',
			text: 'Pastikan alasan izin sudah tepat',
			icon: 'warning',
    ...
    const save = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>long/save',
                    method: 'POST',
                    data: $('#form-save-long').serialize(),
                    dataType: 'JSON',
                    beforeSend: () => {
                        $('#loading').show()
                    },
